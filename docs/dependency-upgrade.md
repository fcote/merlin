...
These packages use the newest version supported by their peer dependencies rather than forcing an unsupported dependency tree:

| Package | Selected | Registry latest | Reason |
| --- | --- | --- | --- |
| GraphQL, client and server | 16.14.2 | 17.0.2 | Apollo Server 5, TypeGraphQL and graphql-subscriptions require GraphQL 16. Keeping both applications on the same version also preserves the schema contract. |
| graphql-scalars, server | 1.26.0 | 2.0.0 | TypeGraphQL requires `^1.25.0`. |
| TypeScript, both applications | 6.0.3 | 7.0.2 | The latest typescript-eslint parser supports TypeScript `<6.1.0`. |
| ESLint, client | 9.39.5 | 10.10.0 | The latest eslint-plugin-react supports ESLint through v9. The server uses ESLint 10. |
